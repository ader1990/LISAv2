...
function Main {
    ...
    foreach ($p in $params) {
        ...
        if ($tokens.Length -ne 2) {
            LogWarn "Warn: test parameter '$p' is being ignored because it appears to be malformed"
            continue
        }
    ...
    LogMsg "VM ${vmName} is present on server and running"
    ...
    if (-not $osInfo) {
        LogErr "Unable to collect Operating System informatioin"
        return "FAIL"
    }
    ...
    LogMsg "VM ${vmName} has imported back successfully"
