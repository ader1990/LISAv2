...
function Trigger-NmiInterrupt {
    ...
    try {
        ...
        $includeBuildNumberScript = "echo '${VMPassword}' | sudo -S -s eval `"export HOME=``pwd``;echo '${buildNumber}' >> `$HOME/constants.sh`""
        $nmiCheckLog = "/home/${VMUserName}/nmicheck.log"

        RunLinuxCmd -username $VMUserName -password $VMPassword `
                    -ip $Ipv4 -port $VMPort $includeBuildNumberScript -runAsSudo
