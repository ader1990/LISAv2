...
function main() {
    ...
    UpdateSummary "Hyper-V build number: $BuildNumber"
    ...
    do
        if [[ $line = *NMI* ]]; then
            ...
            do
                ...
                UpdateSummary "CPU ${i} interrupt count = ${nmiCount}"
                ...
                if [ $i -eq 0 ] || ([ $BuildNumber -lt 14393 ] && [ $BuildNumber -ge 9600 ]); then
                    if [ $nmiCount -ne 0 ]; then
                        UpdateSummary "NMI received at CPU ${i}"
                    else
                        UpdateSummary "CPU {$i} did not receive a NMI!"
                        SetTestStateFailed
                        exit 10
                    fi
                # only not CPU0 and 2016 (BuildNumber >= 14393) should not receive NMI
                    ...
                    if [ $nmiCount -eq 0 ]; then
                        UpdateSummary "CPU {$i} did not receive a NMI, this is expected"
                    else
                        UpdateSummary "CPU {$i} received a NMI!"
                        SetTestStateFailed
                        exit 10
                    fi
